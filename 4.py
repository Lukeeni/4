...
# --- Custom Goal Builder ---
st.subheader("Custom SMART Goal Builder")

if delayed:
    with st.expander("Create Your Own SMART Goal"):
        selected_sound = st.selectbox("1. Which sound are you targeting?", sorted({s for s, _, _ in delayed}))
        selected_position = st.selectbox("2. What position of the sound is being targeted?", ["initial", "medial", "final"])
        hierarchy_level = st.selectbox(
            "3. Which level of the articulation hierarchy is being targeted?",
            ["Auditory Discrimination", "Isolation", "Syllables", "Words", "Phrases", "Sentences", "Structured Conversation", "Spontaneous Speech"]
        )
        accuracy = st.slider("4. What is the target accuracy percentage?", 50, 100, 80)
        consecutive_sessions = st.number_input("5. Over how many consecutive sessions?", min_value=1, max_value=10, value=3)
        timeframe = st.text_input("6. What is your desired timeframe for goal completion?", "6 weeks")

        custom_goal = (
            f"{child_name} will accurately produce the /{selected_sound}/ sound in the {selected_position} position "
            f"at the {hierarchy_level.lower()} level with {accuracy}% accuracy across "
            f"{consecutive_sessions} consecutive sessions within {timeframe}."
        )

        st.markdown("**SMART Goal Preview:**")
        st.success(custom_goal)
        st.download_button("Download Custom SMART Goal (TXT)", data=custom_goal, file_name="custom_smart_goal.txt", mime="text/plain")
else:
    st.info("Custom goal builder becomes available when delayed sounds are detected.")
